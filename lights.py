...
def light(surface: pygame.Surface, position: pygame_vector2, color: Tuple[int, int, int], radius: int, brightness: int = 1):
    position -= pygame.Vector2(radius, radius)
    for d in range(brightness):
        circle_surf = pygame.Surface((radius*2, radius*2))
        pygame.draw.circle(circle_surf, color, (radius, radius), radius * ( d / brightness))
