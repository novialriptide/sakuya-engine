...
class AI:
    ...
    def __init__(
        self, decisions: typing.Sequence[Decision], update_tick, max_decisions: int = 10
    ):
    ...
    def update_decisions(self, world_ticks_elapsed):
        ...
        """
        if (
            pygame.time.get_ticks() not in self.used_ticks
            and world_ticks_elapsed % self.update_tick == 0
        ):
            decisions_made = 0
            for d in self.decisions:
                if decisions_made >= self.max_decisions:
                    break
