...
def _create_spawn_event_stage(
    ...
    spawn_event = WaitEvent(
        "spawn_enemy",
        wait_time,
        spawn_func,
        args=[event_system, enemy_path, entity_id, lifetime],
    )
    return spawn_event


def load_stage_json(path: str, wave_manager: WaveManager, scene: Scene) -> None:
    file = open(path, "r")
    data = json.load(file)
    wait_time = 0
